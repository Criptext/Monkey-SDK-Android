...
You can add delegates using `MonkeyKit.instance().addDelegate()` method. In this example, a Toast is created everytime a message is received. Each time an event occurs in MonkeyKit, one of these callbacks is executed, so that you can handle it.# Get started
...
You can add delegates using `MonkeyKit.instance().addDelegate()` method. In this example, a Toast is created everytime a message is received. Each time an event occurs in MonkeyKit, one of these callbacks is executed, so that you can handle it.  
